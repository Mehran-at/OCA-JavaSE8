...
/*
    ...
    Whereas, Strings computed by concatenation at run time (if the resultant expression is not constant expression)
    are newly created and therefore distinct.

    fName is a constant variable and lName is a non-constant variable.

    `fName + lName` is not a constant expression and hence the expression will be computed at run-time and the resultant
    String object "JamesGosling" will not be referred by String Pool.

    As fName is constant variable and "Gosling" is String literal, hence the expression `fName + "Gosling"` is a constant
    expression, therefore expression is computed at compile-time and results in String literal "JamesGosling".
    ...
    This means at runtime, variable 'name3' will refer to the same String pool object "JamesGosling", which is referred
    by variable 'name3'.
