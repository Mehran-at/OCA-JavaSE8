package com.oca.arraysandarrayslist.thirtytwo;
        ...
        for (int i = 0 ; i < arr.length ; i++) { //Line n1
            for (int j = 0 ; j < arr[i].length ; j++) { //Line n2
                switch (arr[i][j]) { //Line n3
                    ...
                    case "4":
                    case "6":
                        break; //Line n4. Here break take the control out of switch statement.
                    default:
                        continue; //Line n5. Take the control to the loop update statement j++.
                }
...
/*
    ...
    Please note that break; statement at Line n4 takes the control to Line n6 (outside switch-case block) and not out of
    the inner for loop, whereas, continue statement at Line n5 takes the control to the update expression (j++) of
    Line n2.
    ...
    arr[0][1] = "6", Line n4 is executed, and it takes the control to Line n6. 6 is printed on to the console. Control
    goes to j++ (j = 2). 2 < 3 evaluates to true.

        -3rd iteration of inner loop

      arr[0][2] = "5", Line n5 is executed, and it takes the control to j++ (j = 3).   As 3 < 3 evaluates to false,
      control exits inner loop and goes to i++.
